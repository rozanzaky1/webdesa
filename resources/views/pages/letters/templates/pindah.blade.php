...
<div class="letter-title" style="text-align: center; margin: 30px 0 20px 0;">
    <h3 style="text-decoration: underline; margin-bottom: 10px; font-size: 16px; color: #666;">SURAT KETERANGAN PINDAH</h3>
...
<div class="letter-body" style="font-size: 13px; line-height: 1.8; color: #333;">
    ...
    <table style="width: 100%; margin: 20px 0;" cellpadding="5">
        ...
        @if($letter->additional_data && isset($letter->additional_data['destination_address']))
        <tr>
            <td>Akan Pindah Ke</td>
            <td>:</td>
            <td><span class="editable" data-field="destination_address">{{ $letter->additional_data['destination_address'] }}</span></td>
        </tr>
        @endif
        @if($letter->additional_data && isset($letter->additional_data['move_reason']))
        <tr>
            <td>Alasan Pindah</td>
            <td>:</td>
            <td><span class="editable" data-field="move_reason">{{ $letter->additional_data['move_reason'] }}</span></td>
        </tr>
        @endif
    </table>

    <p style="text-indent: 40px; text-align: justify; margin: 20px 0 10px 0;">
        Adalah benar penduduk Kampung Badran Sari dan telah mengajukan kepindahan. Surat keterangan ini dibuat untuk keperluan:
    </p>
    ...
    <p style="text-indent: 40px; text-align: justify; margin-top: 20px;">
        Demikian surat keterangan ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagai persyaratan administrasi kepindahan.
    </p>
</div>
