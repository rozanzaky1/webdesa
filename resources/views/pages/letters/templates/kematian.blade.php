...
<div class="letter-title" style="text-align: center; margin: 30px 0 20px 0;">
    <h3 style="text-decoration: underline; margin-bottom: 10px; font-size: 16px; color: #666;">SURAT KETERANGAN KEMATIAN</h3>
...
<div class="letter-body" style="font-size: 13px; line-height: 1.8; color: #333;">
    ...
    <table style="width: 100%; margin: 20px 0;" cellpadding="5">
        ...
        @if($letter->additional_data && isset($letter->additional_data['death_date']))
        <tr>
            <td>Tanggal Meninggal</td>
            <td>:</td>
            <td><span class="editable" data-field="death_date">{{ $letter->additional_data['death_date'] }}</span></td>
        </tr>
        @endif
        @if($letter->additional_data && isset($letter->additional_data['death_cause']))
        <tr>
            <td>Sebab Kematian</td>
            <td>:</td>
            <td><span class="editable" data-field="death_cause">{{ $letter->additional_data['death_cause'] }}</span></td>
        </tr>
        @endif
    </table>

    <p style="text-indent: 40px; text-align: justify; margin: 20px 0 10px 0;">
        Telah meninggal dunia dan dimakamkan sesuai dengan keyakinan agama yang dianutnya. Surat keterangan ini dibuat untuk keperluan:
    </p>
